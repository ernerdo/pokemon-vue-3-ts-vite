<script setup lang="ts">
  ...
  const pokemonId: Ref<string | number> = ref(
    Math.floor(Math.random() * 806 + 1)
  )
  const pokemonListName = ref([])

  const getPokemon = () => {
    fetch(`https://pokeapi.co/api/v2/pokemon/${pokemonId.value}`)
      .then((res) => res.json())
      .then((data) => {
        pokemon.name = data.name
        pokemon.sprites.front_default = data.sprites.front_default
  ...
  const getPokemonList = () => {
    fetch(`https://pokeapi.co/api/v2/pokemon?limit=1000`)
      .then((res) => res.json())
      .then((data) => {
        pokemonListName.value = data.results.map(
          (pokemon: { name: string }) => {
            return pokemon.name
          }
        )
        localStorage.setItem(
          'pokemonListName',
          JSON.stringify(pokemonListName.value)
        )
      })
      .catch((err) => {
        console.log(err)
  ...
  const handleSubmit = (pokemonID: string) => {
    error.value = false
    loading.value = true
  ...
  <div class="pokedex">
    <div class="pokedex-left">
      ...
      <div class="pokedex-left-bottom">
        ...
        <PokedexForm @search-pokemon="handleSubmit($event)" />
      </div>
    </div>
  </div>
</template>
